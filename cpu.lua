...
local function offer(a, b, c)
    queue.rear = queue.rear + 1
    queue[queue.rear] = {x = a, y = b, v = c}
end
...
local function isInside(x, y)
    return x > 0 and x < 32 and y > 0 and y < 32
end
...
local function spawnApple()
    while (isImpossible(apple.x, apple.y)) do
        apple.x, apple.y = math.random(31), math.random(31)
...
local function onFrameEvent()
	...
	elseif (interval == 0) then
        ...
        elseif (over < -1) then
            ...
            initArr()
            initQueue()
            offer(apple.x, apple.y, 0)
            arr[apple.x][apple.y] = 0

            while (queue.front < queue.rear) do
                local pos = poll()
                for i = 1, 4 do
                    local x1 = pos.x + const.dir[i].dx
                    local y1 = pos.y + const.dir[i].dy
                    if (isInside(x1, y1) and arr[x1][y1] > 100 and not isImpossible(x1, y1)) then
                        arr[x1][y1] = pos.v + 1
                        offer(x1, y1, arr[x1][y1])
                    end
                end
            end

            local x1, y1, m, d = 0, 0, 101, 0
            for i = 1, 4 do
                local x = X[head] + const.dir[i].dx
                local y = Y[head] + const.dir[i].dy
                if (i ~= oppo(snake.dir) and isInside(x, y) and arr[x][y] < m) then
                    x1 = x
                    y1 = y
                    m = arr[x][y]
                    d = i
            ...
            if (apple.x == x1 and apple.y == y1) then
            ...
            -- for y = 1, 31 do
            --     for x = 1, 31 do print("(" .. x .. ", " .. y .. "): " .. arr[y][x]) end
            -- end
